(function () {
  ...
  function ensurePoolFilter() {
    ...
    tSel.innerHTML = '<option value="">' + TL.escapeHtml(allLabel + " · " + TL.t("editor.traits")) + "</option>" +
      traits.map(function (t) { return '<option value="' + TL.escapeHtml(t) + '">' + TL.escapeHtml(t) + "</option>"; }).join("");
    var cats = [
  ...
  function renderPool() {
    ...
    CHARACTERS.forEach(function (ch) {
      ...
      card.innerHTML =
        ...
        '<div class="cstat">' + TL.t("editor.paranoiaShort") + ch.paranoiaLimit + (ch.traits.length ? TL.t("editor.traitsSep") + TL.escapeHtml(ch.traits.join("/")) : "") + "</div>" +
  ...
  function renderDetail() {
    ...
    var entry = castEntry(ch.id);
    var abHtml = (ch.goodwill || []).map(function (ab) {
    ...
    var spHtml = (ch.specials || []).map(function (s) {
      return '<div class="ab" style="color:#8fc7ff;">' + TL.escapeHtml(s) + "</div>";
    }).join("");
    var html = '<div class="detail-card">' +
      ...
      (ch.traits.length ? "　" + TL.t("editor.traits") + " " + TL.escapeHtml(ch.traits.join("、")) : "") +
      ...
      "</div>" +
      '<div style="color:var(--text-dim);font-size:14px;">' + TL.escapeHtml(ch.desc) + "</div>" +
  ...
  function renderMainPlots() {
    ...
    mod.mainPlots.forEach(function (pid) {
      ...
      div.innerHTML = '<div class="name">' + TL.escapeHtml(TL.pname(pid)) + "</div>" +
        '<div class="desc">' + TL.escapeHtml(p.desc) + "</div>" +
  ...
  function renderIncidents() {
    ...
    SCRIPT.incidents.forEach(function (inc, idx) {
      ...
      var effectCell = def ? '<td style="text-align:left;font-size:15px;color:var(--text-dim);max-width:260px;">' + TL.escapeHtml(def.desc) + "</td>" : "<td>—</td>";
    ...
    TL.UI.modal({
      ...
      body: function (bodyEl) {
        ...
        function renderRefList() {
          ...
          list.innerHTML = "";
          var items = refTabData(currentTab, search);
      ...
      buttons: [{ label: "關閉", value: "close" }]
    });
  }

  function refTabData(tabIdx, search) {
    var out = [];
    ...
    if (tabIdx === 0) {
      MAIN_PLOTS.forEach(function (p) {
        if (!pass(TL.pname(p.id) + p.desc)) return;
        out.push({
          name: TL.pname(p.id),
      ...
      INCIDENTS.forEach(function (inc) {
        if (!pass(TL.iname(inc.id) + inc.desc)) return;
        out.push({
          name: TL.iname(inc.id),
          meta: TL.t("editor.moduleTag", { m: modTag(inc.module) }) + (inc.extraCondition ? " · " + TL.t("editor.extraCond", { desc: inc.extraCondition.desc }) : ""),
          desc: inc.desc
        });
      });
    } else if (tabIdx === 4) {
      CHARACTERS.forEach(function (ch) {
        if (!pass(TL.cname(ch.id) + (ch.traits || []).join("") + (ch.specials || []).join("") + (ch.goodwill || []).map(function (g) { return g.desc; }).join(""))) return;
        var abDesc = (ch.goodwill || []).map(function (g) {
          return TL.t("editor.gwCost", { n: g.cost }) + (g.oncePerLoop ? TL.t("editor.perLoopOnce") : "") + (g.cannotBeRefused ? TL.t("editor.cannotRefuse") : "") + " " + g.desc;
        }).join("<br>") || TL.t("editor.tempNoGoodwill");
        var spDesc = (ch.specials || []).map(function (s) { return TL.t("editor.specials") + s; }).join("<br>");
        out.push({
          name: TL.cname(ch.id),
          meta: TL.t("editor.moduleTag", { m: modTag(ch.module) }) + " · " + TL.t("editor.paranoiaLimit") + " " + ch.paranoiaLimit +
            (ch.traits.length ? " · " + TL.t("editor.traits") + " " + ch.traits.join("、") : "") +
